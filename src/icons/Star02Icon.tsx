import * as React from 'react';

export interface Star02IconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
}

const Star02Icon = React.forwardRef<SVGSVGElement, Star02IconProps>(
  ({ size = 24, color = 'currentColor', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path d="M11.4951 2.7143C11.7017 2.29575 12.2985 2.29575 12.5051 2.7143L15.1791 8.13242C15.2611 8.29863 15.4196 8.41383 15.6031 8.44048L21.5823 9.30932C22.0442 9.37644 22.2286 9.94406 21.8944 10.2698L17.5678 14.4873C17.4351 14.6166 17.3745 14.803 17.4058 14.9857L18.4272 20.9408C18.5061 21.4008 18.0233 21.7516 17.6101 21.5345L12.2621 18.7228C12.0981 18.6366 11.9021 18.6366 11.738 18.7228L6.39002 21.5345C5.97689 21.7516 5.49404 21.4008 5.57294 20.9408L6.59432 14.9857C6.62565 14.803 6.56509 14.6166 6.43236 14.4873L2.10573 10.2698C1.7715 9.94406 1.95594 9.37644 2.41783 9.30932L8.39708 8.44048C8.5805 8.41383 8.73906 8.29863 8.82109 8.13242L11.4951 2.7143Z" stroke={color} strokeWidth="2" strokeLinejoin="round"/>
    </svg>
  )
);

Star02Icon.displayName = 'Star02Icon';

export { Star02Icon };
export default Star02Icon;
