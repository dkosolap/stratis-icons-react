import * as React from 'react';

export interface UserProfileCircleIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
}

const UserProfileCircleIcon = React.forwardRef<SVGSVGElement, UserProfileCircleIconProps>(
  ({ size = 24, color = 'currentColor', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path d="M5.3999 19.1999C5.86107 18.6834 8.02095 16.3066 8.65391 16.3066H15.3463C16.2635 16.3066 18.1359 18.2768 18.5999 18.9713M21.5999 11.9999C21.5999 17.3018 17.3018 21.5999 11.9999 21.5999C6.69797 21.5999 2.3999 17.3018 2.3999 11.9999C2.3999 6.69797 6.69797 2.3999 11.9999 2.3999C17.3018 2.3999 21.5999 6.69797 21.5999 11.9999ZM15.4387 8.72786C15.4387 6.89635 13.8926 5.3999 12.0002 5.3999C10.1078 5.3999 8.56164 6.89635 8.56164 8.72786C8.56164 10.5594 10.1078 12.0558 12.0002 12.0558C13.8926 12.0558 15.4387 10.5594 15.4387 8.72786Z" stroke={color} strokeWidth="2"/>
    </svg>
  )
);

UserProfileCircleIcon.displayName = 'UserProfileCircleIcon';

export { UserProfileCircleIcon };
export default UserProfileCircleIcon;
